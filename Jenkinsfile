  ...
  stages {
    ...
    stage('Run aaa-server for Integration Test'){
      ...
      post{
        failure{
          script{
            sh 'mvn flyway:clean -Dflyway.configFiles=/home/ubuntu/configs/tgdex/aaa-flyway.conf'
            sh 'docker compose -f docker-compose-test.yml down --remove-orphans'
          }
    ...
    stage('Integration Test & OWASP ZAP pen test'){
      steps{
        ...
        script{
            sh 'cp /home/ubuntu/configs/tgdex/aaa-config-integ.json configs/config-integ.json'
